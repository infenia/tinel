#!/usr/bin/env python3
"""Output Formatters for Infenix CLI.

This module provides various output formatting capabilities for the Infenix CLI,
including text, JSON, and YAML formats with color support.
...
class YAMLFormatter(BaseFormatter):
    ...
    def format(self, data: Any, title: Optional[str] = None) -> str:
        ...
        if not YAML_AVAILABLE:
            raise RuntimeError("YAML formatting requires PyYAML. Install with: pip install PyYAML")
...
class TextFormatter(BaseFormatter):
    ...
    def _format_minimal_dict(self, data: Dict[str, Any], prefix: str = "") -> str:
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                lines.extend(self._format_minimal_dict(value, f"{full_key}.").split('\n'))
            elif isinstance(value, list):
                for i, item in enumerate(value):
                    if isinstance(item, dict):
                        lines.extend(self._format_minimal_dict(item, f"{full_key}[{i}].").split('\n'))
                    else:
    ...
    def _format_dict(self, data: Dict[str, Any], indent: int = 0) -> List[str]:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                lines.append(f"{indent_str}{self.colorizer.colorize(key + ':', Color.BOLD_WHITE)}")
                ...
                lines.append(f"{indent_str}{self.colorizer.colorize(key + ':', Color.BOLD_WHITE)} {formatted_value}")
    ...
    def _format_list(self, data: list, indent: int = 0) -> List[str]:
        ...
        for i, item in enumerate(data):
            if isinstance(item, dict):
                lines.append(f"{indent_str}{self.colorizer.colorize(f'[{i}]:', Color.YELLOW)}")
                ...
                lines.append(f"{indent_str}{self.colorizer.colorize('-', Color.YELLOW)} {formatted_value}")
    ...
    def _add_verbose_info(self, data: Dict[str, Any]) -> List[str]:
        ...
        lines = [""]
        lines.append(self.colorizer.colorize("Verbose Information:", Color.BOLD_MAGENTA))
...
class FormatterFactory:
    ...
    @staticmethod
    def create_formatter(format_type: FormatType, colorizer=None, quiet: bool = False, verbose: int = 0) -> BaseFormatter:
...
class TableFormatter:
    ...
    def _format_data_lines(self, data: list, headers: list, col_widths: List[int]) -> List[str]:
...
class OutputFormatter:
    ...
    def format_with_explanation(self, data: Any, explanation: str, title: Optional[str] = None) -> str:
    ...
    def print_output_with_explanation(self, data: Any, explanation: str, title: Optional[str] = None) -> None:
