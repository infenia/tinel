#!/usr/bin/env python3
"""Command Line Argument Parser for Infenix.

This module provides comprehensive command line argument parsing for the
Infenix system diagnostics and hardware analysis tool.
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog='infenix',
        description='Infenix - AI-powered Linux system diagnostics and hardware analysis',
        epilog='For more information, visit: https://github.com/infenia/infenix',
    ...
    subparsers = parser.add_subparsers(
        ...
        description='Use "infenix <command> --help" for command-specific help',
...
def _add_hardware_commands(subparsers) -> None:
    ...
    cpu_parser = hw_subparsers.add_parser(
        'cpu',
        parents=[hardware_parent],
        help='CPU information and analysis'
    )
    cpu_parser.add_argument(
        '--temperature',
        action='store_true',
        help='Include temperature information'
    )
    cpu_parser.add_argument(
        '--features',
        action='store_true',
        help='Show CPU features and capabilities'
    )
...
def validate_arguments(args) -> bool:
    ...
    if args.command in ['hardware', 'hw']:
        if hasattr(args, 'hardware_command') and not args.hardware_command:
            print("Error: Hardware command requires a subcommand. Use 'infenix hardware --help' for options.", file=sys.stderr)
            return False
    
    if args.command == 'kernel':
        if hasattr(args, 'kernel_command') and not args.kernel_command:
            print("Error: Kernel command requires a subcommand. Use 'infenix kernel --help' for options.", file=sys.stderr)
            return False
    
    if args.command == 'logs':
        if hasattr(args, 'logs_command') and not args.logs_command:
            print("Error: Logs command requires a subcommand. Use 'infenix logs --help' for options.", file=sys.stderr)
            return False
    
    if args.command in ['diagnose', 'diag']:
        if hasattr(args, 'diag_command') and not args.diag_command:
            print("Error: Diagnose command requires a subcommand. Use 'infenix diagnose --help' for options.", file=sys.stderr)
            return False
    
    if args.command == 'server':
        if hasattr(args, 'server_command') and not args.server_command:
            print("Error: Server command requires a subcommand. Use 'infenix server --help' for options.", file=sys.stderr)
            return False
