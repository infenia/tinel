...
class TestCPUAnalyzerPerformance:
    ...
    def _setup_realistic_mocks(self, slow_mode=False):
        ...
        def mock_run_command(cmd):
            ...
            if cmd[0] == "nproc":
                ...
                return TestDataBuilder.create_command_result(
    ...
    def test_memory_usage_bounds(self):
        ...
        large_cpuinfo = (
            "processor : 0\nmodel name : Test CPU\n"
            + "flags : "
            + " ".join([f"flag{i}" for i in range(1000)])
            + "\n"
        )
        large_lscpu = "Architecture: x86_64\nCPU(s): 4\n" + "\n".join(
            [f"Field{i}: Value{i}" for i in range(100)]
        )
    ...
    def test_file_reading_performance(self):
        ...
        with (
            ...
            for size in file_sizes:
                ...
                # Reading should be fast (under 50ms per 100KB, more lenient)
                max_time = max(
                    0.05 * (size / (100 * 1024)), 0.005
                )  # At least 5ms minimum
    ...
    def test_large_data_formatting_performance(self):
        ...
        large_data = {
            "cpu_info": {
                ...
                "features": {
                    f"feature{i}": i % 2 == 0 for i in range(100)
                },  # 100 features
            },
            "detailed_info": {
                f"key{i}": f"value{i}" * 10 for i in range(50)
            },  # 50 detailed entries
        }
