...
Handle 0x0002, DMI type 17, 40 bytes
	...
	Size: 16 GB
"""
DMIDECODE_UNINSTALLED = "Handle 0x0001, DMI type 17, 40 bytes\n\tSize: No Module Installed"
...
class TestMemoryAnalyzerFinal:
    ...
    def test_parse_dmidecode_variants(self, analyzer):
        assert len(analyzer._parse_dmidecode_output(DMIDECODE_FULL)['memory_devices']) == 1
        assert len(analyzer._parse_dmidecode_output(DMIDECODE_MULTI_INSTALLED)['memory_devices']) == 2
        assert not analyzer._parse_dmidecode_output(DMIDECODE_UNINSTALLED)['memory_devices']
        assert not analyzer._parse_dmidecode_output(DMIDECODE_GARBAGE)['memory_devices']
        assert not analyzer._parse_dmidecode_output(DMIDECODE_OTHER_DEVICE)['memory_devices']
    ...
    def test_full_success(self, mock_swap, mock_virtual, analyzer, mock_si):
        ...
        mock_si.run_command.return_value = CommandResult(True, DMIDECODE_FULL, "", 0, None)
    ...
    def test_psutil_both_fail(self, mock_swap, mock_virtual, analyzer, mock_si):
        ...
        assert "virtual fail" in info['psutil_error'] and "swap fail" in info['psutil_error']
    ...
    def test_psutil_swap_fail_only(self, mock_swap, analyzer, mock_si):
        ...
        assert 'psutil_error' in info
        assert "swap fail" in info['psutil_error'] and "virtual" not in info['psutil_error']
    ...
    def test_dmidecode_command_fail_no_error(self, analyzer, mock_si):
        ...
        with patch('psutil.virtual_memory'), patch('psutil.swap_memory'):
            info = analyzer.get_memory_info()
        assert 'dmidecode_error' in info and "Failed to run dmidecode" in info['dmidecode_error']
    ...
    def test_dmidecode_parse_fail(self, analyzer, mock_si):
        ...
        with patch.object(analyzer, '_parse_dmidecode_output', side_effect=Exception("parse error")):
            with patch('psutil.virtual_memory'), patch('psutil.swap_memory'):
                info = analyzer.get_memory_info()
    ...
    def test_psutil_virtual_fail_only(self, analyzer, mock_si):
        mock_si.run_command.return_value = CommandResult(True, "", "", 0, None)
        with patch('psutil.virtual_memory', side_effect=Exception("virtual fail")), patch('psutil.swap_memory') as s:
    ...
    def test_psutil_error_concatenation(self, analyzer, mock_si):
        ...
        with patch('psutil.virtual_memory', side_effect=Exception("virtual fail")), \
            ...
            info = analyzer.get_memory_info()
        assert 'psutil_error' in info
        assert info['psutil_error'] == 'virtual_memory: virtual fail; swap_memory: swap fail'
