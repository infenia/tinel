#!/usr/bin/env python3
"""Error Handler for Infenix CLI.

This module provides comprehensive error handling for the Infenix CLI,
including meaningful error messages and appropriate exit codes.
...
class CLIErrorHandler:
    ...
    def __init__(self, formatter: OutputFormatter):
        ...
        self.error_suggestions = {
            ExitCode.COMMAND_NOT_FOUND: "Use 'infenix --help' to see available commands",
            ExitCode.INVALID_ARGUMENT: "Check command syntax with 'infenix <command> --help'",
    ...
    def create_error_report(
        ...
        report = {
            ...
            "system": {
                ...
                "infenix_version": "0.1.0",  # This should be imported from version module
            },
            "context": context or {},
        }
    ...
    def save_error_report(
        ...
        filename = f"infenix_error_{timestamp}.json"
    ...
    def validate_system_requirements(self) -> None:
        ...
        if platform.system() != "Linux":
            raise ConfigurationError(
                f"Infenix requires Linux, found {platform.system()}", "operating_system"
            )
    ...
    def handle_permission_escalation(self, operation: str, resource: str) -> None:
        ...
        raise PermissionError(
            ...
            f"Try running with elevated privileges: sudo infenix {operation}",
            resource,
        )
    ...
    def validate_cli_arguments(self, args: Any) -> None:
        ...
        if (hasattr(args, 'quiet') and hasattr(args, 'verbose') and 
            isinstance(args.quiet, bool) and isinstance(args.verbose, int) and
            args.quiet and args.verbose > 0):
            raise InvalidArgumentError(
                "Cannot use both --quiet and --verbose options together"
            )

        # Validate verbosity level
        if hasattr(args, 'verbose') and isinstance(args.verbose, int) and args.verbose < 0:
            raise InvalidArgumentError(
                "Verbosity level cannot be negative",
                'verbose'
            )

        # Validate output format (only if it's a real string, not a Mock)
        valid_formats = ['text', 'json', 'yaml']
        if hasattr(args, 'format') and isinstance(args.format, str) and args.format not in valid_formats:
            raise InvalidArgumentError(
    ...
    def format_error_for_user(self, error: Exception, context: Optional[str] = None) -> str:
