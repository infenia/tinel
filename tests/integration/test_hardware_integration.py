...
class TestCPUAnalyzerIntegration:
    ...
    def _setup_complete_system_mock(self, cpuinfo, lscpu):
        ...
        def mock_run_command(cmd):
            ...
            return command_responses.get(cmd_key, CommandResult(
...
"""
        ...
        command_responses = {
            'lscpu': CommandResult(success=True, stdout='Architecture: x86_64\n', stderr='', returncode=0),
    ...
    def _verify_complete_cpu_info(self, cpu_info):
        ...
        AssertionHelpers.assert_contains_keys(cpu_info, [
        ...
        for feature_set in ['security_features', 'performance_features', 'virtualization_features']:
...
class TestDeviceAnalyzerIntegration:
    ...
    def test_get_all_hardware_info(self, sample_cpuinfo, sample_lscpu):
        ...
        with patch.object(self.device_analyzer.cpu_analyzer, 'get_cpu_info', return_value=cpu_data):
    ...
    def test_cpu_info_delegation(self):
        ...
        with patch.object(self.device_analyzer.cpu_analyzer, 'get_cpu_info', return_value=expected_cpu_info):
...
class TestHardwareToolsIntegration:
    ...
    def test_cpu_tool_provider(self, sample_cpuinfo):
        ...
        with patch.object(cpu_tool.device_analyzer, 'get_cpu_info', return_value=mock_cpu_data):
    ...
    def test_all_hardware_tool_provider(self):
        ...
        with patch.object(all_hw_tool.device_analyzer, 'get_all_hardware_info', return_value=mock_hardware_info):
...
class TestHardwareCommandsIntegration:
    ...
    def setup_method(self):
        ...
        self.hardware_commands = HardwareCommands(self.mock_formatter, self.mock_error_handler)
    ...
    def test_cpu_command_execution(self):
        ...
        # Mock tool execution
        mock_cpu_data = {'model': 'Test CPU', 'cores': 4}
        with patch.object(self.hardware_commands.cpu_tool, 'execute', return_value=mock_cpu_data):
    ...
    def test_all_hardware_command_execution(self):
        ...
        with patch.object(self.hardware_commands.all_hardware_tool, 'execute', return_value=mock_hw_data):
            result = self.hardware_commands.execute(args)
    ...
    def test_command_error_handling(self):
        ...
        with patch.object(self.hardware_commands.cpu_tool, 'execute', side_effect=RuntimeError("Test error")):
...
class TestEndToEndWorkflow:
    ...
    def _setup_realistic_system_mock(self, mock_system, cpuinfo, lscpu):
        ...
        def mock_run_command(cmd):
            return cmd_data.get(cmd[0], CommandResult(
...
@pytest.mark.parametrize("command_name,expected_title", [
    ('cpu', 'CPU Information'),
...
def test_command_title_mapping(command_name, expected_title):
    ...
    if command_name == 'cpu':
        with patch.object(hardware_commands.cpu_tool, 'execute', return_value=mock_data):
            hardware_commands.execute(args)
    else:
        with patch.object(hardware_commands.all_hardware_tool, 'execute', return_value=mock_data):
            hardware_commands.execute(args)
