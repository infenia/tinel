...
class TestCLIErrorHandler(unittest.TestCase):
    ...
    def test_validate_system_requirements_missing_commands(self):
        ...
        with patch('platform.system', return_value='Linux'), \
             patch('sys.version_info', (3, 11, 0)), \
             patch.object(self.error_handler, 'validate_command_availability', side_effect=side_effect):
    ...
    def test_validate_cli_arguments_valid_format(self):
        """Test CLI argument validation with valid format."""
        args = Mock()
        args.format = 'json'
        args.verbose = 1
        args.quiet = False
        ...
        # Should not raise any exception
        self.error_handler.validate_cli_arguments(args)

    def test_validate_cli_arguments_invalid_format(self):
        """Test CLI argument validation with invalid format."""
        args = Mock()
        args.format = 'invalid'
        ...
        self.assertIn("Invalid output format", cm.exception.message)
        self.assertEqual(cm.exception.details["argument"], "format")

    def test_validate_cli_arguments_negative_verbosity(self):
        """Test CLI argument validation with negative verbosity."""
        args = Mock()
        args.verbose = -1
        ...
        self.assertIn("Verbosity level cannot be negative", cm.exception.message)

    def test_validate_cli_arguments_conflicting_options(self):
        """Test CLI argument validation with conflicting options."""
        args = Mock()
        args.quiet = True
        args.verbose = 1
        ...
        self.assertIn("Cannot use both --quiet and --verbose", cm.exception.message)

    def test_get_error_context(self):
        """Test error context gathering."""
        context = self.error_handler.get_error_context()

        self.assertIn("timestamp", context)
        self.assertIn("system", context)
        
        # Check that we have some basic system information
        if "error" not in context:  # Only if we successfully gathered context
    ...
    def test_format_error_for_user(self):
        ...
        self.assertEqual(formatted, "ValueError: Test error message")
        
        formatted_with_context = self.error_handler.format_error_for_user(error, "test context")
        self.assertEqual(formatted_with_context, "test context: ValueError: Test error message")

    @patch("sys.exit")
    @patch("sys.stderr", new_callable=StringIO)
    def test_handle_error_with_report_saving(self, mock_stderr, mock_exit):
        """Test error handling with error report saving."""
        details = {"operation": "test", "data": "value"}
        
        with patch.object(self.error_handler, 'save_error_report') as mock_save:
            mock_save.return_value = "/tmp/error_report.json"
            
            self.error_handler.handle_error(
                "Test error", ExitCode.GENERAL_ERROR, details
            )
        
        mock_save.assert_called_once()
        stderr_output = mock_stderr.getvalue()
        self.assertIn("Error report saved to: /tmp/error_report.json", stderr_output)
