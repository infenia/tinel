...
class AssertionHelpers:
    ...
    def assert_no_sensitive_data(
        data: Any, sensitive_patterns: Optional[List[str]] = None
    ):
        ...
        found_patterns = [
            pattern for pattern in sensitive_patterns if pattern in data_str
        ]
...
class PerformanceTestHelpers:
    ...
    @staticmethod
    def assert_cached_performance(
        cached_call, uncached_call, min_improvement: float = 2.0
    ):
