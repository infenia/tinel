...
class TestCLIErrorHandler:
    ...
    def test_handle_error_save_report(self):
        ...
        with patch('sys.exit'):
            with patch.object(self.handler, 'save_error_report', return_value="/tmp/report.json"):
                with patch('sys.stderr', new_callable=StringIO) as mock_stderr:
    ...
    def test_handle_cli_error(self):
        ...
        cli_error = CLIError("Test CLI error", ExitCode.HARDWARE_ERROR, {"component": "cpu"})
    ...
    def test_validate_system_requirements_missing_commands(self):
        ...
        with patch('platform.system', return_value="Linux"):
            with patch('sys.version_info', (3, 11, 0)):
                with patch.object(self.handler, 'validate_command_availability', side_effect=CommandNotFoundError("lscpu")):
                    with pytest.raises(ConfigurationError, match="Required system utilities not found"):
                        self.handler.validate_system_requirements()
...
class TestArgumentValidation:
    ...
    def test_validate_cli_arguments_conflicting_quiet_verbose(self):
        ...
        with pytest.raises(InvalidArgumentError, match="Cannot use both --quiet and --verbose"):
    ...
    def test_validate_cli_arguments_negative_verbosity(self):
        ...
        with pytest.raises(InvalidArgumentError, match="Verbosity level cannot be negative"):
            self.handler.validate_cli_arguments(args)
...
@pytest.mark.parametrize("exit_code,has_suggestion", [
...
@pytest.mark.parametrize("error_class,expected_exit_code", [
...
def test_error_exit_codes(error_class, expected_exit_code):
    ...
    elif error_class in [InvalidArgumentError, NetworkError, ConfigurationError, 
                        HardwareError, KernelError, LogAnalysisError, DiagnosticsError]:
