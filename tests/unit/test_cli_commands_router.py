...
class TestCommandRouter:
    ...
    def test_execute_command_cli_error_logging(self):
        ...
        with patch('tinel.cli.commands.router.logger') as mock_logger:
            ...
            mock_logger.debug.assert_called_with("CLI error in command hardware: Test error")
    ...
    def test_execute_command_unexpected_error_logging(self):
        ...
        with patch('tinel.cli.commands.router.logger') as mock_logger:
            ...
            mock_logger.exception.assert_called_with("Unexpected error executing command hardware")
...
@pytest.mark.parametrize("command,should_exist", [
    ("hardware", True),
    ("hw", True),
    ("unknown", False),
    ("", False),
...
def test_command_handler_existence(command, should_exist):
    ...
    if should_exist:
        ...
        assert command not in router.command_handlers or router.command_handlers.get(command) is None
...
def test_exception_handling_types(exception_type, expected_result):
    ...
    with patch('tinel.cli.commands.router.HardwareCommands') as mock_hw_class:
        ...
        error_handler.handle_exception.assert_called_once_with(exception_type, "command 'hardware'")
